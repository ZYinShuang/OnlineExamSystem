			...
			$("#shortAnswerDetail").dialog({    
			    ...
			    buttons:[{
					...
					handler:function(){
						...
						console.log(stage);
						console.log(isOk);
						if($("#isShow").combobox("getValue")==""||$("#course").val()==""
								||isOk == 0){
							$.messager.alert('失败提示','所有信息不可为空，请确认！');
						}else{
							$.post("course/submitCourse.do",
									{
										'cid':$("#id").val(),
										'course':$("#course").val(),
		...
		function delProblem(){
			...
			if(array.length==0){
				...
				$.messager.confirm("删除提示","删除数据将导致关联课程失效！确定删除"+array.length+"条数据",function(isTrue){
					if(isTrue){
						$.post("course/delCourse.do",{'arr':arr},function(data){
							if(data=="true"){
  ...
  <body>
	...
	<table id="ListTable">   
	    <thead>   
	        <tr>   
	            ...
	            <th data-options="field:'isShow_show',align:'center'" width="200">课程标志</th>      
	        </tr>   
	    </thead>  
	</table> 
	...
	<!-- 
	 ...
	 <div id="toolbar" style="padding-bottom: 10px">
	 		...
	 		<a href="#" class="easyui-linkbutton" iconCls="icon-add" plain="true"  onclick="addProblem()">新增课程</a>&nbsp;
	 		<a href="#" class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="delProblem()">删除课程</a>&nbsp;
	 		<!-- 
	 		<a href="#" class="easyui-linkbutton" iconCls="icon-edit" plain="true" onclick="updateProblem()">修改课程</a>&nbsp;
	 		 -->
	 		 &nbsp;&nbsp;&nbsp;&nbsp;
		 		课程名称：<input type="text" name="s_title" class="inputCls" id="s_title">&nbsp;
