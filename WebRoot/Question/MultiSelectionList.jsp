	...
	<script type="text/javascript">
		...
		$(function(){
			$('#ListTable').datagrid({
				...
				//width:1160,
				fitColumns:true,
				url:'selection/searchSelection.do?type=2',
				...
				onDblClickRow:function(rowIndex, rowData){
					...
					$.post("selection/searchOption.do",{'pid':rowData.id},function(data){
						...
						$.each(data,function(index,item){
							$("#optionList").append("<tr><td style='text-align: right;'><a href='javascript:;' onclick='deleteOption(this)' >删除</a><input type='checkbox' name='isAnw' "+(item.anw?"checked='checked'":"")+"'></td><td colspan='5'><input type='text' name='option' value="+item.option+" class='font' /><input type='hidden' name='optId' value="+item.id+" /></td></tr>");
						});	
			...
			$("#shortAnswerDetail").dialog({    
			    ...
			    buttons:[{
					...
					handler:function(){
						...
						$.each(isAnws,function(index,item){
							if(item.checked){
								anwNum++;
								anwArr.push(true);
							}else{
						...
						}else if(isAnws.length<2||options.length<2){
							$.messager.alert('失败提示','选项请至少2项！');
						}else if(anwNum < 1){
							$.messager.alert('失败提示','至少选择一项正确答案！');
						}else if(isOptOk == 0){
							$.messager.alert('失败提示','所有选项不可为空，请确认！');
						}else{
		...
		function delProblem(){
			...
			if(array.length==0){
				...
				$.messager.confirm("删除提示","确定删除"+array.length+"条数据",function(isTrue){
					if(isTrue){
						$.post("selection/delSelection.do?type=2",{'arr':arr},function(data){
							if(data=="true"){
		...
		//新增一个选项
		function addOption(){
			$("#optionList").append("<tr><td style='text-align: right;'><a href='javascript:;' onclick='deleteOption(this)' >删除</a><input type='checkbox' name='isAnw'></td><td colspan='5'><input type='text' name='option' value='' class='font' /><input type='hidden' name='optId' value='0' /></td></tr>");
		}
	</script>
	...
	<div id="shortAnswerDetail">
			...
			<tr>
				...
				<tr >
					...
					<td>
						<a href="#" class="easyui-linkbutton" iconCls="icon-add" onclick="addOption()">添加选项</a>
					</td>
				</tr>
				<tbody id="optionList">
