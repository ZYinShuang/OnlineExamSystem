	...
	<script type="text/javascript">
		...
		$(function(){
			$('#ListTable').datagrid({
				...
				//width:1160,
				fitColumns:true,
				url:'selection/searchSelection.do?type=1',
				...
				onDblClickRow:function(rowIndex, rowData){
					...
					$.post("selection/searchOption.do",{'pid':rowData.id},function(data){
						...
						$.each(data,function(index,item){
							$("#optionList").append("<tr><td style='text-align: right;'><a href='javascript:;' onclick='deleteOption(this)' >删除</a><input type='radio' name='isAnw' "+(item.anw?"checked='checked'":"")+"'></td><td colspan='5'><input type='text' name='option' value="+item.option+" class='font' /><input type='hidden' name='optId' value="+item.id+" /></td></tr>");
						});	
			    ...
			    buttons:[{
					...
					handler:function(){
						...
						if($("#cid").combobox("getValue")==""||$("#nycd").combobox("getValue")==""
							...
							$.messager.alert('失败提示','必须选择一项正确答案！');
						}else if(isOptOk == 0){
		...
		function delProblem(){
			...
			if(array.length==0){
				...
				$.messager.confirm("删除提示","确定删除"+array.length+"条数据",function(isTrue){
					if(isTrue){
						$.post("selection/delSelection.do?type=1",{'arr':arr},function(data){
							if(data=="true"){
		...
		//新增一个选项
		function addOption(){
			$("#optionList").append("<tr><td style='text-align: right;'><a href='javascript:;' onclick='deleteOption(this)' >删除</a><input type='radio' name='isAnw'></td><td colspan='5'><input type='text' name='option' value='' class='font' /><input type='hidden' name='optId' value='0' /></td></tr>");
		}
	</script>
